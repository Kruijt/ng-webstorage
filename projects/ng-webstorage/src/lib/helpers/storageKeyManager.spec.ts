...
describe('Helpers : StorageKeyManager', () => {
  ...
  it('should determine if the given key is a managed key', () => {
    ...
    expect(
      StorageKeyManager.isNormalizedKey(`${StorageKeyManager.prefix}--random_key`)
    ).toBeFalsy();
  ...
  it('should normalize the given key', () => {
    expect(StorageKeyManager.normalize('RANDOM_KEY')).toEqual(
      `${StorageKeyManager.prefix}${StorageKeyManager.separator}random_key`
    );
