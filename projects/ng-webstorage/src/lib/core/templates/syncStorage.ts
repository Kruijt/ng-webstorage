...
export class SyncStorage implements StorageService {
  ...
  retrieve(key: string): any {
    let value: any;
    this.strategy
      .get(StorageKeyManager.normalize(key))
      .subscribe(result => (value = typeof result === 'undefined' ? null : result));
    return value;
  }
  ...
  clear(key?: string): void {
    if (key !== undefined) this.strategy.del(StorageKeyManager.normalize(key)).subscribe(noop);
